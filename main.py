import uvicorn
from fastapi import FastAPI, HTTPException, UploadFile, File, Form, Request, Header, Body
...
# Add after your SimulationParams class
class SimulationParamsWithCaptcha(BaseModel):
    ms_id: int
    kappa1: float
    alpha: float
    captcha_id: str
...
async def get_captcha():
    ...
    return {"captcha_id": captcha_id, "image": f"data:image/png;base64,{image_base64}"}
...
@app.api_route("/voila/{path:path}", methods=["GET", "POST", "PUT", "DELETE"])
@app.api_route("/voila", methods=["GET", "POST", "PUT", "DELETE"])
async def proxy_voila(request: Request, path: str = "", captcha_token: str = Cookie(None)):
    # Validate captcha_token as you do for /api/simulate
    now = datetime.now()
    expiry = captcha_tokens.get(captcha_token)
    if not captcha_token or not expiry or expiry < now:
        return JSONResponse({"error": "CAPTCHA required"}, status_code=403)

    # Forward the request to the actual Voila server
    voila_url = f"http://localhost:8888/{path}" if path else "http://localhost:8888/"

    # Forward cookies, especially _xsrf
    cookies = request.cookies.copy()
    headers = {k: v for k, v in request.headers.items() if k.lower() != "host"}

    # Forward X-XSRFToken header if present
    if "_xsrf" in cookies:
...
# Serve the original simulator HTML file
@app.get("/index.html", response_class=HTMLResponse)
async def get_simulator_html():
